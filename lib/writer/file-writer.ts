...
export function writeTypesToFile(types: string[], outputPath: string): void {
  ...
  // Load the types base file, removing initial comments
  const typesBasePath = `${__dirname}/types-base.ts`;
  const typesBase = fs.readFileSync(typesBasePath, 'utf8').split('\n');
  ...
  const typesBaseContents = typesBase.slice(i).join('\n');

  toWrite += '// START types-base.ts\n';
  toWrite += typesBaseContents;
  toWrite += '// END types-base.ts\n\n';
